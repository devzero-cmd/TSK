...
# Directories for saving captured images
...
# Keep track of scanned QR to avoid duplicates
scanned_qrs = set()
...
def extract_name_from_vcard(qr_text):
    """Extract N: field from VCARD QR code"""
    if not qr_text:
        return None
    match = re.search(r"N:([^\n\r]+)", qr_text)
    if match:
...
def sample_filename(qr_text):
    """Return filename for captured_samples using QR name or timestamp fallback"""
...
def capture_filename():
    """Return filename for captured_images"""
...
def capture_image_page():
    """Capture single image as sample and save with name from QR"""
    image_url = None
...
def capture():
    """Capture normal webcam image into captured_images folder"""
    frame_bytes = camera.get_frame()
    if frame_bytes is None:
